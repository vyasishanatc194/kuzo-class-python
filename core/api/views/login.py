...
class ForgotPasswordAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if user:
                current_site = "{}/verify-link/".format('http://127.0.0.1:8000') 
                final_current_site = "{}{}".format(current_site, token) 

                email = Emails(subject="Forgot Password???", recipient_list=request.data["email"], )
                email.set_html_message('forgot_password/user.html', {'user': request.data["email"],'site':final_current_site})
                email.send()
                return Response({"data":{},"status":True,"message":"You will receive a link to create a new password via email. Please check your email","code":status.HTTP_200_OK}, status=status.HTTP_200_OK)

        except User.DoesNotExist:
            return Response({"data":{},"status":False,"message":"Email is not registered","code":status.HTTP_400_BAD_REQUEST}, status=status.HTTP_400_BAD_REQUEST)
...
class ChangePasswordLinkCheckAPIView(APIView):
    permission_classes = (AllowAny,)

    def get(self, request, uuid_string, **kwargs):
        try:
            ...
            if user:
                ...
                return Response({"data":[],"status":False,"message":"Bad request","code":status.HTTP_400_BAD_REQUEST}, status=status.HTTP_400_BAD_REQUEST)
...
class SetPasswordAPIView(APIView):
    ...
    def put(self, request, uuid_string, format=None):
        ...
        try:
            ...
            user.set_password(new_password1)
            user.save()
            return Response({"data":user.id,"status":True,"message":"Successfully changed password.","code":status.HTTP_200_OK}, status=status.HTTP_200_OK)

        except User.DoesNotExist:
            return Response({"data":[],"status":False,"message":"Bad request, cannot change password","code":status.HTTP_400_BAD_REQUEST}, status=status.HTTP_400_BAD_REQUEST)
